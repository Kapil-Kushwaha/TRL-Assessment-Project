...
class TRLApp:
    def __init__(self, master):
        self.master = master
        master.title("TRL Assessment Tool")

        # Set up main frame with background color
        self.main_frame = tk.Frame(master, bg="#f7f7f7")
        self.main_frame.pack(fill=tk.BOTH, expand=True)

        # Add a logo or image (You need to replace 'logo_path.png' with your actual image path)
        try:
            self.logo = PhotoImage(file="logo_path.png")  # Replace 'logo_path.png' with your image path
            self.logo_label = tk.Label(self.main_frame, image=self.logo, bg="#f7f7f7")
            self.logo_label.pack(pady=20)
        except Exception as e:
            print(f"Could not load image: {e}")
        
        # Title heading
        self.title_label = tk.Label(self.main_frame, text="Technology Readiness Level (TRL) Assessment", font=("Arial", 18, "bold"), bg="#f7f7f7")
        self.title_label.pack(pady=10)

        # Short description
        self.description = tk.Label(self.main_frame, text="Select the technology type to begin the TRL assessment process.", font=("Arial", 12), bg="#f7f7f7")
        self.description.pack(pady=10)

        # Technology type selection dropdown
        self.tech_types = [
        ...
        self.submit_button = tk.Button(self.main_frame, text="Start Assessment", command=self.open_questionnaire, font=("Arial", 12, "bold"), bg="#4CAF50", fg="white")
